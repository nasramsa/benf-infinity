...
export default function CommanderPage() {
  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = apiError.response?.data?.message || apiError.response?.data?.errors?.email?.[0] || apiError.message || 'Identifiants ou données incorrects.';
  ...
  const handleCreateOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      const apiError = err as ApiError;
      setAddressError(apiError.response?.data?.message || 'Erreur lors de la création de la commande. Veuillez vérifier les stocks.');
    } finally {
